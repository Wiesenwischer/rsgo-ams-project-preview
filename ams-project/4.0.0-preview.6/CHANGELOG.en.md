...
> **Note:** These release notes cover the current preview state and have so far been generated up to and including **4.0.0-preview.6**. They will be extended with the upcoming preview/RC builds through the final 4.0.0.
...
- Detailed step-by-step guide: see the [calendar guide](../Guides/features/kalender.md).
...
**Additional fixes (preview builds preview.3 – preview.6)**
- **Login/session:** after a longer idle period, instead of a technical error ("Correlation failed" / HTTP 500) a friendly **"session expired"** page is shown with a fresh sign-in; re-login no longer loops through a redirect.
- **Schedule (Gantt):** **resizing a task's duration by dragging** now honours the **calendar** (weekends/holidays); the duration shown in the tooltip matches the stored one (off-by-one fixed).
- **Detached detail window:** is fully usable again (clicks and input) and carries the localized title **"Task-Details"**.
- **Desktop startup:** the Windows **taskbar icon** is shown again; the "loading configuration" splash text is localized; the bootstrap configuration is retried automatically on a timeout.
- **Print preview** (reporting) opens in an in-app window with the correct project association.
- **Operations:** the platform service and installer start reliably even with certain log-level settings (no more crash).
