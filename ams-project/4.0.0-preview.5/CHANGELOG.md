...
> **Hinweis:** Diese Release Notes decken den aktuellen Vorschau-Stand ab und wurden bisher bis einschließlich **4.0.0-preview.4** generiert. Sie werden mit den weiteren Preview-/RC-Ständen bis zur finalen 4.0.0 fortgeschrieben.
...
- **Warnungen aktualisieren sich live**: Änderungen an Einschränkungen, Terminen oder Abhängigkeiten aktualisieren die Indikator-Spalte und den Warnungszähler in der Statusleiste sofort – ohne den Plan neu laden zu müssen. Warnungen werden dabei der tatsächlichen Ursache zugeordnet (kein doppeltes Melden desselben Konflikts an mehreren Vorgängen).
...
- Detaillierte Schritt-für-Schritt-Anleitung: siehe Kalender-Guide.
...
- **Ersteller und Erstelldatum** eines Terminplans werden erfasst und in der Projektliste angezeigt.
...
- **Vorgänger-Tab erweitert**: ein **Konfliktzähler** als Badge am Tab, **Konflikt-Symbole je Konfliktart** mit erklärenden Tooltips, die **Einheit des Zeitabstands** an den Feldern, **Doppelklick** auf einen Vorgänger springt zu ihm, und das **Entfernen eines Vorgängers lässt sich rückgängig machen**.
- **Aufgabentitel per Klick umbenennen** direkt in der Detailansicht.
...
- **Kapazitätsplanung** mit neuen Web-Buchungsdialogen (Iststunden, Fertigstellungsgrad, Fertigungskennung); das Ressourcen-Grid zeigt zusätzlich **Auftrag und Position**, und die Personalnummer lässt sich per Link (URL-Parameter) vorbelegen.
...
- **Visuelle Rückmeldung** in der Gantt: kopierte und eingefügte Vorgänge werden kurz hervorgehoben, sodass sofort erkennbar ist, was kopiert bzw. wohin eingefügt wurde.
...
- **Sammel-Revision**: alle aktiven Projekte lassen sich in einem Durchlauf aus dem Portal revisionieren (Menüpunkt „Revisionen").
...
- Die Sitzung übersteht jetzt einen **Neustart des Servers**: Sicherheitsschlüssel und Sitzungsspeicher liegen zentral, sodass ein Neustart nicht mehr alle Benutzer abmeldet. Die Sitzung bleibt zudem über die reine Token-Lebensdauer hinaus aktiv.
- Der Desktop-Client versucht bei abgelaufener Anmeldung zunächst eine **stille erneute Anmeldung**, bevor er nachfragt; vorübergehende Verbindungsfehler zum Server werden abgefangen und als **freundliche Meldung** angezeigt, statt mit einem Fehlerdialog abzubrechen.
...
- Negativer Puffer wird der **tatsächlichen Ursache** zugeordnet: Vorgänger bzw. Nachfolger einer unerfüllbaren „Spätestens"-Einschränkung erhalten keine eigene, irreführende Puffer-Warnung mehr.
- Der **Warnungszähler in der Statusleiste stimmt mit den Warn-Symbolen in der Gantt überein** – derselbe Konflikt wird nicht mehr doppelt gezählt.
- Warnungen erscheinen **live**, ohne den Plan neu zu laden.
- Ein **Anfang-Anfang-Nachfolger** eines Tagesend-Meilensteins behält den gemeinsamen Starttermin; eine **Daueränderung an einem manuell geplanten Vorgang** terminiert dessen Ende korrekt neu.
...
- Behobene Endlosschleife bei der Kapazitätsberechnung großer Arbeitspläne; die Kapazitäts-Übersicht ist chronologisch sortiert und fällt bei Überlast korrekt je Arbeitsplatz zurück.
...
- Stücklisten-Liste sortiert die Vorgangs-ID jetzt numerisch statt alphanumerisch; die Stücklisten-Ansicht öffnet nicht mehr leer (veraltete gespeicherte Seitengröße), und die Zuordnungs-Tabellen sind auf 500 Zeilen begrenzt.
- Behobener **Startabsturz des Desktop-Clients** (Statusleiste der Zwischenablage).
- Pläne **ohne Aufgaben** zeigen jetzt ihre Menüs und einen leeren Detailbereich statt eines hängenden Zustands; das Flackern der Detailansicht bei identischen Live-Aktualisierungen ist behoben.
- Der **Konflikte-/Warnungsbereich** und das **WPF-Menü** folgen dem Dark Mode; verschiedene Layout-Korrekturen an der Projektliste (Platz für die Bildlaufleiste, Kontextmenü in der zusammengeführten Status-Spalte, Grid bleibt in der Fensterbreite).

**Zusammenarbeit**
- Notiz- und Diskussionsbereiche wurden mit dem einheitlichen Design-System überarbeitet: lesbare Sprechblasen, jederzeit aktiver Senden-Knopf, kein Aufflackern beim Öffnen und keine überflüssige Bildlaufleiste mehr.

**Administrationsportal**
- Sammel-Revision aller aktiven Projekte aus dem Portal; der mandant.ini-Import bringt Produktiv-Voreinstellungen mit und leitet das ERP-Stammverzeichnis ab.
